...
const Blog = () => {
    ...
    const addComment = async (e) => {
        e.preventDefault();
        try {
            const { data } = await axios.post('/api/blog/add-blogComment', { blog: id, name, content })
            if (data.success) {
    ...
    return data ? (
        <div className="relative ">
            ...
            <div className='text-center text-gray-600'>
                ...
                <p className='inline-block py-1 my-6 px-4 rounded-full mb-6 border text-sm border-primary/35 bg-primary/5 font-medium text-primary'>{data.creator?.name || "Unknown Author"}</p>
            </div>
            <div className='mx-5 max-w-5xl md:mx-auto my-5'>
                <img src={data.image} alt="" className='rounded-3xl my-5 w-[80%] mx-auto block' />

                <div className='rich-text max-w-3xl mx-auto' dangerouslySetInnerHTML={{ __html: data.description }}></div>

                {/* comment section */}
                <div className='mt-5 mb-5 max-w-3xl mx-auto'>
                ...
                <div className='max-w-3xl mx-auto'>
                    ...
                    <form onSubmit={addComment} className='flex flex-col items-start gap-4 max-w-lg'>
                        ...
                        <button onClick={() => fetchComments()} type='submit' className='bg-primary text-white rounded p-2 px-8 hover:scale-102 transition-all cursor-pointer'>Submit</button>
                    </form>
                </div>
                {/* share buttons */}
